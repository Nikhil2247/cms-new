...
// User IDs from the mentors
const mentorMongoIds = [
...
async function main() {
  ...
  // Find students with the duplicate roll numbers (the ones that WERE migrated)
  ...
  // Get all students and check for potential duplicates by similar email or name patterns
  ...
  for (const student of allStudents) {
    ...
    for (const a of assignments) {
      console.log(`│    - Mentor: ${a.Mentor?.name}, Active: ${a.isActive}, Assigned: ${a.assignmentDate}`);
    }
  }
  console.log('└─');

  // Now let's parse the MongoDB backup to get info about the duplicate records
  console.log('\n' + '═'.repeat(80));
  ...
  // Find the duplicate students in the backup
  ...
  // Find students with those MongoDB IDs
  ...
  // Also find all students that share the same userId
  ...
  const failedApp = allDocs.find(doc =>
    getIdString(doc._id) === failedAppId
  );
